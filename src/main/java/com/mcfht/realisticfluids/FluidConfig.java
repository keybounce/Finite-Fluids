...
public class FluidConfig {
	...
	public static void handleConfigs(Configuration config)
	{
		...
		RealisticFluids.FAR_UPDATES = config.getInt("globalFarUpdates", "1 - General", 2048, 0, 10000000, "Quota for trivial updates");
		RealisticFluids.MAX_UPDATES = config.getInt("globalNearUpdates", "1 - General", 1024, 0, 10000000, "Immediate update Factor. WIP! Currently not implemented at all");
		
		RealisticFluids.GLOBAL_RATE_MAX = config.getInt("globalMaxUpdateInterval", "1 - General", 10, 3, 64, "The largest allowed number ticks between each update sweep");
		RealisticFluids.GLOBAL_RATE_AIM = config.getInt("globalIdealUpdateInterval", "1 - General", 5, 1, 64, "The ideal number of ticks between each update sweep");
		...
		RealisticFluids.EQUALIZE_GLOBAL	= 5 * config.getInt("globalEqualizeCap", "2 - Equalization", 8, 1, 1024, "Max Equalizations per tick. More causes faster equalization. Sane values: 4 ~ 32");
